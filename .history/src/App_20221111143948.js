      ...
      <div className="weather-container">
        ...
        <Forecast>
          {weather.forecast.map(item => {
            return <ForecastItem key={item.date} value={item.day.avgtemp_c} icon={getWeatherIcon(weather.current.is_day, weather.current.condition.text)} text={item.day.condition.text} />
          })}
        </Forecast>
      </div>
      : <Loading /> }
