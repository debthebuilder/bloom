...
function App() {
  ...
  const bgColor = weather.current?.is_day || 0;
  return ( 
    <div className="app" style={{backgroundColor: {weather.current?.is_day === 1 ? "white" : "black"}}}>
      {!loading ?
          ...
          {weather.forecast.map(item => {
            return(
              <ForecastItem 
                ...
                icon={getWeatherIcon(weather.current.is_day, item.day.condition.text)} 
