...
function App() {
  ...
  useEffect(() => {
    axios.request("https://weatherapi-com.p.rapidapi.com/forecast.json", {
        ...
        if(response) {
            setWeather({
                current: response.data.current.forecastday,
  ...
  return (
    <div className="app">
      ...
      <div className="weather-container">
        <Current currentDate={moment().format('MMMM Do YYYY, h:mm:ss a')} location={`${weather.location.location.name}, ${weather.location.location.country}`} />
        <Forecast />
      </div>
      : <Loading /> }
    </div>
